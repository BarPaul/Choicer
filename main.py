...
Formatter.converter = lambda *_: datetime.now(timezone('Europe/Moscow')).timetuple()
...
def start_command(message: types.Message):
    ...
    if user is None:
        logger.info(f"{message.from_user.full_name} ({message.from_user.id}) зарегистрировался")
...
def info_command(message: types.Message):
    ...
    if user is None:
        return bot.reply_to(message, "Ваши данные отсутсвуют в боте! Нажмите /start, чтоб обновить данные")
    if message.text == 'ℹ️ Информация':
        ...
        logger.info(f"{message.from_user.full_name} ({message.from_user.id}) вернулся в главное меню")
        return bot.reply_to(message, "Ты вернулся в главное меню", reply_markup=MENU)
        ...
        def answer_easy(message: types.Message):
            if message.text == '◀️ Вернуться':
                logger.info(f"{message.from_user.full_name} ({message.from_user.id}) возращается в игровое меню")
                user['in_game'] = False
                return bot.reply_to(message, "Ты вернулся в игровое меню", reply_markup=GAME)
            if message.text not in ["🟥 Красный", "⬛ Черный"]:
                logger.info(f"{message.from_user.full_name} ({message.from_user.id}) выбирает неверный цвет")
            ...
            logger.info(f"{message.from_user.full_name} ({message.from_user.id}) выбирает {message.text}, пока программа выбрала {ans_card}")
            if isrght:
                logger.info(f"{message.from_user.full_name} ({message.from_user.id}) получает 5 очков за победу в легком уровне")
                user['points'] += 5
                user['right'] += 1
                bot.send_photo(
                ...
                update_data()
                return bot.register_next_step_handler_by_chat_id(message.chat.id, answer_easy)
            bot.send_photo(chat_id=message.chat.id,
                            reply_to_message_id=message.id,
                            caption="❌ Не угадал!",
                            photo=open(f'./{ans_card[0]}/{ans_card[1:]}.png', 'rb'))
            logger.info(f"{message.from_user.full_name} ({message.from_user.id}) поражен в легком уровне")
            return bot.register_next_step_handler_by_chat_id(message.chat.id, answer_easy)

        return bot.register_next_step_handler_by_chat_id(message.chat.id, answer_easy)
    elif message.text == '🍌 Средний':
        logger.info(f"{message.from_user.full_name} ({message.from_user.id}) выбирает средний уровень")
        user['in_game'] = True
        keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
        keyboard.add("♦️ Бубны", "♥️ Червы", '♠️ Пики', '♣️ Трефы')
        keyboard.add("◀️ Вернуться")
        bot.reply_to(message, "Я загадал карту. Угадай масть :)", reply_markup=keyboard)
        ...
        def answer_middle(message: types.Message):
            ...
            if message.text not in ["♦️ Бубны", "♥️ Червы", '♠️ Пики', '♣️ Трефы']:
                logger.info(f"{message.from_user.full_name} ({message.from_user.id}) выбирает неверную масть")
            ...
            isrght, ans_card = is_right(user_card)
            logger.info(f"{message.from_user.full_name} ({message.from_user.id}) выбирает {message.text}, а программа выбрала {ans_card}")
            if isrght:
                logger.info(f"{message.from_user.full_name} ({message.from_user.id}) получает 20 очков за победу в среднем уровне")
                user['points'] += 20
                user['right'] += 1
                bot.send_photo(
                    chat_id=message.chat.id,
                    ...
                    photo=open(f'./{ans_card[0]}/{ans_card[1:]}.png', 'rb'))
                update_data()
                return bot.register_next_step_handler_by_chat_id(message.chat.id, answer_middle)
            logger.info(f"{message.from_user.full_name} ({message.from_user.id}) поражен в среднем уровне")
            bot.send_photo(chat_id=message.chat.id,
                        reply_to_message_id=message.id,
                        caption="❌ Не угадал!",
                        photo=open(f'./{ans_card[0]}/{ans_card[1:]}.png', 'rb'))
        ...
        logger.info(f"{message.from_user.full_name} ({message.from_user.id}) выбирает сложный уровень")
        user['in_game'] = True
        bot.reply_to(message, "Я загадал карту. Угадай карту :)", reply_markup=CARDS)
        ...
        def answer_hard(message: types.Message):
            ...
            if message.text not in cards.keys():
                logger.info(f"{message.from_user.full_name} ({message.from_user.id}) выбирает неверную карту")
            ...
            logger.info(f"{message.from_user.full_name} ({message.from_user.id}) выбирает {message.text}, пока программа выбрала {ans_card}")
            if isrght:
                logger.info(f"{message.from_user.full_name} ({message.from_user.id}) выигрывает 50 очков за победу в сложном уровне")
                user['points'] += 50
                user['right'] += 1
                bot.send_photo(
                    chat_id=message.chat.id,
            ...
            logger.info(f"{message.from_user.full_name} ({message.from_user.id}) поражен в сложном уровне")
            bot.send_photo(chat_id=message.chat.id,
                        reply_to_message_id=message.id,
                        caption="❌  Не угадал!",
                        photo=open(f'./{ans_card[0]}/{ans_card[1:]}.png', 'rb'))
